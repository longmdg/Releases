namespace SparkTech.Base
{
    ...
    public static class xSaliceWalker
    {
        ...
        private static bool Drawing => Helper.UsedOrbwalker == Orbwalker.xSaliceWalker;
        ...
        private static int humanizerTick;
        private static int windup;
        private static int lastRealAttack;

        public static void AddToMenu(Menu mennu)
        {
            movementPrediction = new Spell(SpellSlot.Unknown, GetAutoAttackRange());
            movementPrediction.SetTargetted(Player.BasicAttack.SpellCastTime, Player.BasicAttack.MissileSpeed);
            ...
            mennu.AddSubMenu(menuModes);

            mennu.AddItem(new MenuItem("AnimationCheck", "Animation Check").SetValue(true));
            mennu.AddItem(new MenuItem("MissileCheck", "Missile Check").SetValue(true));
            mennu.AddItem(new MenuItem("OrbwalkingTargetMode", "Mode").SetValue(new StringList(new[] { "To Mouse", "To Target" })));
            mennu.AddItem(new MenuItem("orb_Misc_AutoWindUp", "Autoset Windup").SetValue(new KeyBind("O".ToCharArray()[0], KeyBindType.Press)));

            LeagueSharp.Drawing.OnDraw += OnDraw;
            Game.OnUpdate += OnUpdate;
            Obj_AI_Base.OnProcessSpellCast += OnProcessSpell;
            GameObject.OnCreate += MissileClientOnOnCreate;
        ...
        {
            if (menu.Item("MissileCheck").GetValue<bool>() && sender.IsMe && !sender.IsMelee && args.Animation != "Run" && args.Animation == "Idle" && (args.Animation.Contains("Attack") || args.Animation == "Crit"))
            {
                Utility.DelayAction.Add((int)(Player.AttackCastDelay * 100 + Game.Ping), () => canmove = true);
            }
        }

        private static void MissileClientOnOnCreate(GameObject sender, EventArgs args)
        {
            if (!menu.Item("MissileCheck").GetValue<bool>())
                return;

            var missile = sender as MissileClient;

            if (missile == null || !missile.SpellCaster.IsMe || !Orbwalking.IsAutoAttack(missile.SData.Name)
        ...
        private static void OnProcessSpell(Obj_AI_Base unit, GameObjectProcessSpellCastEventArgs spell)
        {
            ...
            if (Orbwalking.IsAutoAttackReset(spell.SData.Name))
        ...
        private static void OnUpdate(EventArgs args)
        {
            ...
            if (Utils.GameTimeTickCount - humanizerTick < menu.Item("orb_Misc_Humanizer").GetValue<Slider>().Value)
                return;

            var target = GetTarget();

            var mode = menu.Item("OrbwalkingTargetMode").GetValue<StringList>().SelectedIndex;

            Orbwalk(target, mode != 0 && target is Obj_AI_Hero ? target.Position : Game.CursorPos);
        }
        ...
        private static void OnDraw(EventArgs args)
        {
            ...
            {
                Render.Circle.DrawCircle(Player.Position, GetAutoAttackRange(), menu.Item("orb_Draw_AARange").GetValue<Circle>().Color);
            }
            ...
            foreach (var enemy in HeroManager.Enemies.Where(enemy => enemy.IsValidTarget(1500)))
            {
                if (menu.Item("orb_Draw_AARange_Enemy").GetValue<Circle>().Active)
                    Render.Circle.DrawCircle(
                        enemy.Position,
                        GetAutoAttackRange(enemy, Player),
                ...
                if (menu.Item("orb_Draw_hitbox").GetValue<Circle>().Active)
                    Render.Circle.DrawCircle(
                        ...
                        menu.Item("orb_Draw_hitbox").GetValue<Circle>().Color);
            }
            ...
            if (menu.Item("orb_Draw_AARange_Enemy").GetValue<Circle>().Active)
            {
                foreach (var enemy in HeroManager.Enemies.Where(enemy => enemy.IsValidTarget(1500)))
                {
                    Render.Circle.DrawCircle(enemy.Position, GetAutoAttackRange(enemy, Player), menu.Item("orb_Draw_AARange_Enemy").GetValue<Circle>().Color);
                }
            }

            if (menu.Item("orb_Draw_Holdzone").GetValue<Circle>().Active)
            {
                Render.Circle.DrawCircle(Player.Position, menu.Item("orb_Misc_Holdzone").GetValue<Slider>().Value, menu.Item("orb_Draw_Holdzone").GetValue<Circle>().Color);
            }
        ...
        public static void Orbwalk(AttackableUnit mytarget, Vector3 goalPosition)
        {
            ...
            var @base = mytarget as Obj_AI_Base;
            if (@base != null)
            {
                var target = @base;

                if ((CanAttack || HaveCancled) && IsAllowedToAttack())
                {
                    ...
                    if (!disableNextAttack)
                    {
                        if (CurrentMode != Mode.Harass || !target.IsMinion ||
                            menu.Item("Harass_Lasthit").GetValue<bool>())
                        {
                ...
                {
                    movementPrediction.Delay = Player.BasicAttack.SpellCastTime;
                    movementPrediction.Speed = Player.BasicAttack.MissileSpeed;
        ...
        private static void MoveTo(Vector3 position)
        {
            ...
            if (Utils.GameTimeTickCount - humanizerTick < delay)
                return;

            humanizerTick = Utils.GameTimeTickCount;
        ...
        private static AttackableUnit GetTarget()
        {
            ...
            if (menu.Item("orb_Misc_Priority_Unit").GetValue<StringList>().SelectedIndex == 1 && (CurrentMode == Mode.Harass || CurrentMode == Mode.LaneClear))
            {
        ...
        private static AttackableUnit GetBaseStructures()
        {
            ...
            foreach (var barrack in
                ObjectManager.Get<Obj_BarracksDampener>().Where(t => t.IsValidTarget(GetAutoAttackRange(Player, t)))) return barrack;
        ...
        private static AttackableUnit GetBestMinion(bool lastHitOnly)
        {
            ...
            if (menu.Item("NewMode").GetValue<bool>())
            {
                var turret =
                ...
                if (turret != null && lastHitOnly)
                {
                    foreach (
                        var minion in
                            from minion in
                                enemies.Where(x => turret.Distance(x.ServerPosition) < 1000)
                                .OrderBy(x => x.Distance(turret))
                            let playerProjectile = ProjectTime(minion)
                            let predHealth = HealthPrediction.GetHealthPrediction(minion, playerProjectile, FarmDelay())
                            let turretProjectile =
                                turret.AttackCastDelay * 1000
                                + turret.Distance(minion) / turret.BasicAttack.MissileSpeed * 1000
                            where !(predHealth < 0) && !(playerProjectile * 1.8 > turretProjectile)
                            where
                                predHealth - turret.GetAutoAttackDamage(minion)
                                - Player.GetAutoAttackDamage(minion, true) * 2 <= 0
                                && predHealth - turret.GetAutoAttackDamage(minion)
                                - Player.GetAutoAttackDamage(minion, true) > 0
                                && predHealth - turret.GetAutoAttackDamage(minion) * 2 < 0
                            select minion) return minion;
                }
            }
        ...
        private static int FarmDelay(int offset = 0) => offset != 0 ? offset : menu.Item("orb_Misc_Farmdelay").GetValue<Slider>().Value;
        ...
        public static bool CanAttack => lastAaTick <= Utils.GameTimeTickCount &&
                                        (Utils.GameTimeTickCount + Game.Ping / 2 + 25 >= lastAaTick + Player.AttackDelay * 1000 && attack);

        private static bool HaveCancled => lastAaTick - Utils.GameTimeTickCount > Player.AttackCastDelay * 1000 + 25 && lastRealAttack < lastAaTick;

        public static bool CanMove(float delay) => CanMoves;

        private static bool CanMoves => canmove || lastAaTick <= Utils.GameTimeTickCount &&
                                        (Utils.GameTimeTickCount + Game.Ping / 2 >= lastAaTick + Player.AttackCastDelay * 1000 + windup &&
                                         movement);

        private static float MyProjectileSpeed => Player.CombatType == GameObjectCombatType.Melee ? float.MaxValue : Player.BasicAttack.MissileSpeed;

        public static void SetAttack(bool value) => attack = value;

        public static void SetMovement(bool value) => movement = value;

        public static double CountKillhits(Obj_AI_Base enemy) => enemy.Health / Player.GetAutoAttackDamage(enemy);

        public static int GetCurrentWindupTime() => menu.Item("orb_Misc_ExtraWindUp").GetValue<Slider>().Value;
