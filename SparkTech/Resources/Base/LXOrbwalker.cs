namespace SparkTech.Resources.Base
{
    ...
    internal class LXOrbwalker
    {
        ...
        private static void OnDraw(EventArgs args)
        {
            ...
            if (!Menu.Item("lxOrbwalker_Draw_MinionHPBar").GetValue<Circle>().Active && !Menu.Item("lxOrbwalker_Draw_Lasthit").GetValue<Circle>().Active && !Menu.Item("lxOrbwalker_Draw_nearKill").GetValue<Circle>().Active)
            return;
            ...
            foreach (var minion in minionList.Where(minion => minion.IsValidTarget(GetAutoAttackRange() + 500)))
            {
                ...
                if (Menu.Item("lxOrbwalker_Draw_MinionHPBar").GetValue<Circle>().Active)
                {
                    for (var i = 1; i < attackToKill; i++)
                    {
                        ...
                        Drawing.DrawLine(new Vector2(startposition, hpBarPosition.Y + 18), new Vector2(startposition, hpBarPosition.Y + 23), Menu.Item("lxOrbwalker_Draw_MinionHPBar_thickness").GetValue<Slider>().Value, Menu.Item("lxOrbwalker_Draw_MinionHPBar").GetValue<Circle>().Color);
                    }
                }
                if (Menu.Item("lxOrbwalker_Draw_Lasthit").GetValue<Circle>().Active && minion.Health <= MyHero.GetAutoAttackDamage(minion, true))
                Render.Circle.DrawCircle(minion.Position, minion.BoundingRadius, Menu.Item("lxOrbwalker_Draw_Lasthit").GetValue<Circle>().Color);
                else if (Menu.Item("lxOrbwalker_Draw_nearKill").GetValue<Circle>().Active && minion.Health <= MyHero.GetAutoAttackDamage(minion, true) * 2)
        ...
        public static void Orbwalk(Vector3 goalPosition, Obj_AI_Base target)
        {
            ...
            return;
            if (MyHero.IsMelee && target != null && target.Distance(MyHero) < GetAutoAttackRange(MyHero, target) && Menu.Item("lxOrbwalker_Melee_Prediction").GetValue<bool>() && target is Obj_AI_Hero && Game.CursorPos.Distance(target.Position) < 300)
            {
        ...
        private static void MoveTo(Vector3 position, float holdAreaRadius = -1)
        {
            ...
            if (Environment.TickCount - _lastMovement < delay || !CanMove())
            return;
        ...
        private static void OnProcessSpell(Obj_AI_Base unit, GameObjectProcessSpellCastEventArgs spell)
        {
            ...
            if (unit.IsMe)
            {
                ...
                if (unit.IsMelee)
                Utility.DelayAction.Add((int)(unit.AttackCastDelay * 1000 + Game.Ping * 0.5) + 50, () => FireAfterAttack(unit, _lastTarget));
            }
        ...
        public static double GetAzirAASandWarriorDamage(Obj_AI_Base unit)
        {
            ...
            int count = ObjectManager.Get<Obj_AI_Minion>().Count(obj => obj.Name == "AzirSoldier" && obj.IsAlly && obj.BoundingRadius < 66 && obj.AttackSpeedMod > 1 && obj.Distance(unit) < 350);
            if (count == 2)
            return MyHero.CalcDamage(unit, Damage.DamageType.Magical, dmg) + (MyHero.CalcDamage(unit, Damage.DamageType.Magical, dmg) * 0.25);
        ...
        {
            if (ObjectManager.Get<Obj_Building>().Any(obj => obj.Position.Distance(MyHero.Position) <= GetAutoAttackRange() + obj.BoundingRadius / 2 && obj.IsTargetable && obj.Name.StartsWith("HQ_")))
            return null;
            ...
            if (Menu.Item("lxOrbwalker_Misc_Priority_Unit").GetValue<StringList>().SelectedIndex == 1 && (CurrentMode == Mode.Harass || CurrentMode == Mode.LaneClear))
            {
            ...
            {
                foreach (var turret in ObjectManager.Get<Obj_AI_Turret>().Where(turret => turret.IsValidTarget(GetAutoAttackRange(MyHero, turret))))
                return turret;
                if (ObjectManager.Get<Obj_Building>().Any(obj => obj.Position.Distance(MyHero.Position) <= GetAutoAttackRange() + obj.BoundingRadius / 2 && obj.IsTargetable && (obj.Name.StartsWith("Barracks_") || obj.Name.StartsWith("HQ_"))))
                return null;
            }
        ...
        {
            return ObjectManager.Get<Obj_AI_Minion>().Any(minion => minion.IsValidTarget() && minion.Team != GameObjectTeam.Neutral && InAutoAttackRange(minion) && HealthPrediction.LaneClearHealthPrediction(minion, (int)((MyHero.AttackDelay * 1000) * LaneClearWaitTimeMod), FarmDelay()) <= MyHero.GetAutoAttackDamage(minion));
        }
